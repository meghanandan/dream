...
import {
  ...
  TextField,
  MenuItem,
  Select, Grid, InputLabel, FormControl
} from '@mui/material';
...
const DisputeForm = ({ open, onClose, onSubmit, initialData,setFirst,firstSelect,setDisputeAju,disputeAdjustment }) => {
  const {
    control,
    handleSubmit,
    reset,
  ...
  const handleBlur = (item) => {
              ...
              try {
                  postService(endpoints.auth.getDisputeorAdjustmentDropDown, 'POST', payload).then((res) => {
      ...
      <DialogContent >
        <form onSubmit={handleSubmit(handleFormSubmit)}>
            ...
            <FormControl fullWidth sx={{ mb: 2 }}>
              <InputLabel>{firstSelect===1?'Dispute Template ':'Adjustment Template '}</InputLabel>
              <Select
